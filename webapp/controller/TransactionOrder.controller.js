  ...
  return Controller.extend("com.apptech.app-retention.controller.TransactionOrder", {
	...
	fConfigValueHelpDialogs: function () {
		...
		var sInputValue = this.byId("BPCode").getValue();
			...
			$.ajax({
				url: "https://xsjs.biotechfarms.net/app_xsjs/ExecQuery.xsjs?dbName=" + Database +
	...
	fUPdate: function () {
		...
		$.ajax({
			url: "https://sl-eut.biotechfarms.net/b1s/v1/U_APP_CPOR('" + oCode + "')",
			data: JSON.stringify(oPo),
			type: "PATCH",
			xhrFields: {
				withCredentials: true
	...
	fPOCancellation: function (oDocEntry){
		...
		$.ajax({
			//Posting PO in SAP
			url: "https://sl-eut.biotechfarms.net/b1s/v1/PurchaseOrders("+ oDocEntry +")/Cancel",
			type: "POST",
			xhrFields: {
				withCredentials: true
			},

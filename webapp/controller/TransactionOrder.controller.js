sap.ui.define([
  ...
  return Controller.extend("com.apptech.app-retention.controller.TransactionOrder", {
	...
	fSave: function () {
		...
		if (oVendor === "") {
			...
			if (oRetention === "0") { // YES
				...
				$.ajax({
					//Posting PO in SAP
					url: "https://sl-test.biotechfarms.net/b1s/v1/PurchaseOrders",
	...
	fUPdate: function () {
		...
		$.ajax({
			url: "https://sl-test.biotechfarms.net/b1s/v1/U_APP_CPOR('" + oCode + "')",
			data: JSON.stringify(oPo),
			type: "PATCH",
			xhrFields: {
				withCredentials: true
	...
	fPOCancellation: function (oDocEntry){
		...
		$.ajax({
			//Posting PO in SAP
			url: "https://sl-test.biotechfarms.net/b1s/v1/PurchaseOrders("+ oDocEntry +")/Cancel",
			type: "POST",
			xhrFields: {
				withCredentials: true
			},
